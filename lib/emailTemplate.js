...
function normalizeShortHexColors(html) {
  return String(html || "").replace(/#([0-9a-fA-F]{3})(?![0-9a-fA-F])/g, (match, hex) => {
...
function renderBlock(block, ff, global = {}) {
  ...
  switch (type) {
    ...
    case "image": {
      ...
      const imageHref = normalizeLink(data.link);
    ...
    case "button": {
      const normalizedHref = normalizeLink(data.url);
    ...
    case "footer": {
      ...
      const unsubUrl = esc(normalizeLink(data.unsubUrl) || "#");
...
export function buildEmailHtml({ headline, bodyText, imageUrl, ctaText, ctaLink, accentColor = "#D05A2C" }) {
  const blocks = [
    ...
    ...(ctaText  ? [{ id:"b", type:"button",   data:{ text:ctaText, url:ctaLink||"#", bgColor:accentColor, textColor:"#ffffff", align:"center", size:"large", fullWidth:false, radius:8 } }] : []),
