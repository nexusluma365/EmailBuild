...
export function buildEmailHtmlFromBlocks(blocks = [], global = {}) {
  ...
  const rows = nonSubjectBlocks.map(b => renderBlock(b, fontFamily)).filter(Boolean);
...
<tr><td align="center" style="padding:20px 0 30px;">
  <table role="presentation" border="0" cellpadding="0" cellspacing="0" width="600" style="background-color:#ffffff;max-width:600px;width:100%;box-shadow:0 4px 24px rgba(0,0,0,0.07);">
    ${rows.join("\n")}
  </table>
</td></tr>
</table>
  ...
  switch (type) {
    ...
    case "headline": {
      ...
      return tr(`<td style="padding:24px 32px 10px;font-family:${ff};">
        <h1 style="margin:0;font-size:${data.fontSize||28}px;font-weight:${data.fontWeight||'800'};color:${esc(data.color||'#0f0f1a')};text-align:${align};line-height:1.25;font-family:${ff};">
          ${esc(data.text||'')}
        </h1>
      </td>`);
    }
    ...
    case "button": {
      ...
      const align = data.align || "center";
      const btn = `<a href="${href}" target="_blank" rel="noopener noreferrer" style="display:${data.fullWidth?'block':'inline-block'};background-color:${esc(data.bgColor||'#D05A2C')};color:${esc(data.textColor||'#ffffff')};font-family:${ff};font-size:${fs}px;font-weight:700;text-decoration:none;padding:${pad};border-radius:${data.radius||8}px;text-align:center;mso-padding-alt:0;cursor:pointer;">${esc(data.text||'Click Here')}</a>`;
